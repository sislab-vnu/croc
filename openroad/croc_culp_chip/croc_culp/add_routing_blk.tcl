...
foreach inst $allInsts {
  ...
  if { [string match "*gf180mcu_fd_ip_sram*" $name] } {
  ...
  if { [string match "croc_culp_chip" $name] } {
    set w [$master getWidth]
    set h [$master getHeight]
    ## left side
    set llx_Mx [expr $loc_llx]
    set lly_Mx [expr $loc_lly]
    set urx_Mx [expr $loc_llx + 350.0]
    set ury_Mx [expr $loc_lly + $h]
    ...
    incr cnt
    # right
    set llx_Mx [expr $loc_llx + $w - 350.0]
    set lly_Mx [expr $loc_lly]
    set urx_Mx [expr $loc_llx + $w]
    set ury_Mx [expr $loc_lly + $h]
    ...
    # top
    set llx_Mx [expr $loc_llx]
    set lly_Mx [expr $loc_lly + $h - 350.0]
    set urx_Mx [expr $loc_llx + $w]
    set ury_Mx [expr $loc_lly + $h]
